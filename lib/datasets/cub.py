...
def build_transform(is_train, cfg):
    ...
    t.append(transforms.Normalize(list(map(float, cfg.DATA.IMAGE_MEAN)), list(
        map(float, cfg.DATA.IMAGE_STD))))  # std = [0.229, 0.224, 0.225] mean = [0.485, 0.456, 0.406]
    return transforms.Compose(t)


def get_transforms(cfg):
    train_transform = build_transform(True, cfg)
