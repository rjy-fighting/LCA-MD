...
__all__ = [
    'deit_lca_md_tiny_patch16_224', 'deit_lca_md_small_patch16_224', 'deit_lca_md_base_patch16_224',
    'deit_tscam_tiny_patch16_224', 'deit_tscam_small_patch16_224', 'deit_tscam_base_patch16_224',
    'vit_lca_md_small_patch16_224'
]
...
class TFCM(nn.Module):
    def __init__(self):
        ...
        self.conv3x3 = nn.Conv2d(
            1, 1, kernel_size=3, padding=1, dilation=1
        )
        self.conv3x3_1 = nn.Conv2d(
            200, 200, kernel_size=3, padding=1, dilation=1
        )

    def forward(self, x_patch, cams):
...
class LCA_MD(VisionTransformer):
    def __init__(self, num_layers=2, *args, **kwargs):
        ...
        self.tfcm = TFCM()
        self.from_scratch_layers = [self.tfcm]
        self.num_layers = num_layers
        self.layers = nn.ModuleList()
        d_cpab = D_CPAB(dim=self.num_classes)
    ...
    def forward(self, x, test_select=0):
        ...
        fg_feats, bg_feats, cams = self.tfcm(x_patch, cams)
        ...
        if self.training:
            ...
            return x_logits, fg_feats, bg_feats
        else:
...
@register_model
def deit_lca_md_tiny_patch16_224(pretrained=False, **kwargs):
    model = LCA_MD(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def deit_lca_md_small_patch16_224(pretrained=False, **kwargs):
    model = LCA_MD(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def vit_lca_md_small_patch16_224(pretrained=False, **kwargs):
    model = LCA_MD(
...
@register_model
def deit_lca_md_base_patch16_224(pretrained=False, **kwargs):
    model = LCA_MD(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
